...
router.post("/:id", (req, res) => {
  ...
  //Find restaurant from list of starred restaurants
  const restaurant = STARRED_RESTAURANTS.find((restaurant) => restaurant.id === id);

  //if restaurant doesn't exist, let client know
  if (!restaurant) {
    res.sendStatus(404);
    return;
  }
...
router.delete("/:id", (req, res) => {
  ...
  //remove restaurant from list of started restaurants
  const newStarredRestaruants = STARRED_RESTAURANTS.filter(
    (restaurant) => restaurant.id !== id
  );

  //send status code to client if restaruant doesn't exist
  if (STARRED_RESTAURANTS.length === newStarredRestaurants.length) {
    res.sendStatus(404);
    return;
  }

  STARRED_RESTAURANTS = newStarredRestaruants;
...
router.put("/:id", (req, res) => {
  ...
  //find restaruant in list of starred restaurants
  const restaurant = STARRED_RESTAURANTS.find((restaurant) => restaurant.id === id);
